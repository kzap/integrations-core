...
def assert_metric_counts(aggregator, payload):
    ...
    assert len(aggregator.metrics('{}.bucket'.format(metric_name))) == num_bucket_metrics
    assert len(aggregator.metrics('{}.sum'.format(metric_name))) == num_sum_metrics
    assert len(aggregator.metrics('{}.count'.format(metric_name))) == num_count_metrics
...
def test_default(aggregator, dd_run_check, mock_http_response):
    ...
    check = get_check({'openmetrics_endpoint': 'test', 'metrics': ['.+']})
    dd_run_check(check)

    aggregator.assert_metric(
        'etcd_disk_wal_fsync_duration_seconds.bucket',
        2,
        metric_type=aggregator.MONOTONIC_COUNT,
        tags=['endpoint:test', 'kind:fs', 'app:vault', 'le:0.001'],
    )
    aggregator.assert_metric(
        'etcd_disk_wal_fsync_duration_seconds.sum',
        0.026131671,
        metric_type=aggregator.MONOTONIC_COUNT,
        tags=['endpoint:test', 'kind:fs', 'app:vault'],
    )
    aggregator.assert_metric(
        'etcd_disk_wal_fsync_duration_seconds.count',
        4,
        metric_type=aggregator.MONOTONIC_COUNT,
        tags=['endpoint:test', 'kind:fs', 'app:vault'],
    )
...
def test_disable_histogram_buckets(aggregator, dd_run_check, mock_http_response):
    payload = """
        ...
        """
    mock_http_response(payload)
    check = get_check({'openmetrics_endpoint': 'test', 'metrics': ['.+'], 'collect_histogram_buckets': False})
    dd_run_check(check)
    ...
    aggregator.assert_all_metrics_covered()
    assert len(aggregator.metrics('etcd_disk_wal_fsync_duration_seconds.sum')) == 2
    assert len(aggregator.metrics('etcd_disk_wal_fsync_duration_seconds.count')) == 2
...
def test_non_cumulative_histogram_buckets(aggregator, dd_run_check, mock_http_response):
    payload = """
        ...
        """
    mock_http_response(payload)
    check = get_check({'openmetrics_endpoint': 'test', 'metrics': ['.+'], 'non_cumulative_histogram_buckets': True})
    dd_run_check(check)

    aggregator.assert_metric(
        'rest_client_request_latency_seconds.bucket',
        81,
        metric_type=aggregator.MONOTONIC_COUNT,

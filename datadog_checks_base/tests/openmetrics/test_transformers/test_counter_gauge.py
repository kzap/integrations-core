...
def test(aggregator, dd_run_check, mock_http_response):
    ...
    aggregator.assert_metric(
        'go_memstats_alloc_bytes.total', 9339544592, metric_type=aggregator.GAUGE, tags=['endpoint:test', 'foo:bar']
    )
    aggregator.assert_metric(
    ...
    aggregator.assert_metric(
        'go_memstats_frees.total', 128219257, metric_type=aggregator.GAUGE, tags=['endpoint:test', 'bar:foo']
    )

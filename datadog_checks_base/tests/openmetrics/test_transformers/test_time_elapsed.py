    ...
    check = get_check(
        {
            ...
            'metrics': [{'go_memstats_last_gc_time_seconds': {'type': 'time_elapsed'}}],
        }
    )
    dd_run_check(check)

    assert 'go_memstats_last_gc_time_seconds' in aggregator._metrics
    assert len(aggregator._metrics) == 1
    assert len(aggregator._metrics['go_memstats_last_gc_time_seconds']) == 1
    m = aggregator._metrics['go_memstats_last_gc_time_seconds'][0]

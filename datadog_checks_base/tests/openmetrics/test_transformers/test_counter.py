...
def test_basic(aggregator, dd_run_check, mock_http_response):
    ...
    aggregator.assert_metric(
        'go_memstats_alloc_bytes.count', 9339544592, metric_type=aggregator.MONOTONIC_COUNT, tags=['endpoint:test']
    )
    aggregator.assert_metric(
        'go_memstats_frees.count', 128219257, metric_type=aggregator.MONOTONIC_COUNT, tags=['endpoint:test']
    )
...
def test_tags(aggregator, dd_run_check, mock_http_response):
    ...
    aggregator.assert_metric(
        'go_memstats_alloc_bytes.count',
        9339544592,
        metric_type=aggregator.MONOTONIC_COUNT,
        tags=['endpoint:test', 'foo:bar'],
    )
    aggregator.assert_metric(
        'go_memstats_frees.count',
        128219257,
        metric_type=aggregator.MONOTONIC_COUNT,

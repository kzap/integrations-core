...
def assert_metric_counts(aggregator, payload):
    ...
    assert len(aggregator.metrics('{}.quantile'.format(metric_name))) == num_quantile_metrics
...
def test_default(aggregator, dd_run_check, mock_http_response):
    ...
    aggregator.assert_metric(
        'http_request_duration_microseconds.quantile',
        1599.011,
    ...
    aggregator.assert_metric(
        'http_request_duration_microseconds.count',
        25,
        metric_type=aggregator.MONOTONIC_COUNT,
        tags=['endpoint:test', 'handler:prometheus'],
    )

    ...
    check = get_check(
        {
            ...
            'metrics': [{'state': {'type': 'service_check', 'status_map': {'3': 'ok'}}}],
        }
    )
    dd_run_check(check)

    aggregator.assert_service_check('state', ServiceCheck.OK, tags=['endpoint:test'])
...
def test_unknown(aggregator, dd_run_check, mock_http_response):
    ...
    check = get_check(
        {
            'openmetrics_endpoint': 'test',
            'metrics': [{'state': {'type': 'service_check', 'status_map': {'7': 'ok'}}}],
        }
    )
    dd_run_check(check)

    aggregator.assert_service_check('state', ServiceCheck.UNKNOWN, tags=['endpoint:test'])

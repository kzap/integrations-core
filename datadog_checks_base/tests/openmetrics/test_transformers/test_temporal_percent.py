...
def test_named(aggregator, dd_run_check, mock_http_response):
    ...
    aggregator.assert_metric('process_cpu_usage', 520, metric_type=aggregator.RATE, tags=['endpoint:test', 'foo:bar'])
    ...
    check = get_check(
        {
            'openmetrics_endpoint': 'test',
            'metrics': [
                {'process_cpu_seconds': {'name': 'process_cpu_usage', 'type': 'temporal_percent', 'scale': 1}},
            ],
        }
    )
    dd_run_check(check)
